...
void calc_acc(double *a, double *u, double m, double kappa, double alpha, int size_of_u)
{
    ...
    //a[0] = kappa/m*(- 2.0*u[0] + u[1] + alpha*u[0]*u[1] - alpha*u[0]*u[0] - alpha/2.0*u[1]*u[1]);
    //a[size_of_u-1] = kappa/m*(- 2.0*u[size_of_u-1] + u[size_of_u-2] + alpha*u[size_of_u-1]*u[size_of_u-2] - alpha*u[size_of_u-1]*u[size_of_u-1] - alpha/2.0*u[size_of_u-2]*u[size_of_u-2]);
    
    // what wikipedia says
    a[0] = kappa/m*(- 2.0*u[0] + u[1]) * (1 + alpha*u[1]);
    a[size_of_u-1] = kappa/m*(- 2.0*u[size_of_u-1] + u[size_of_u-2]) * (1 + alpha*(- u[size_of_u-2]));
    
    /* Calculating the acceleration of the inner points */
    for (i = 1; i < size_of_u - 1; i++){
        //a[i] = kappa/m*(- 2.0*u[i] + u[i-1]  + u[i+1] + alpha*u[i]*(u[i-1] + u[i+1]) - alpha*u[i]*u[i] - alpha/2.0*(u[i-1]*u[i-1] + u[i+1]*u[i+1]));
        
        // what wikipedia says
        a[i] = kappa/m*(- 2.0*u[i] + u[i-1]  + u[i+1]) * (1 + alpha*(u[i+1] - u[i-1]));
    }
}
